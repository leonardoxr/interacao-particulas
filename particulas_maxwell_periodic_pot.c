...
double update(struct particulas particula[],double dt,int L,int maxpart)
{
    ...
    for(i=0; i<maxpart; i++)
    {
        ...
        for(j = 0; j<maxpart; j++)
        {
            if(R[j] < (L-1)/2 && r[j][0] < (L-1)/2 && r[j][1] < (L-1)/2)
            {
    ...
    if(x1temp > L - 1)
    {
        particula[i].position[0] = x1temp - (L-1);
    ...
    {
        particula[i].position[0] = L-1 + x1temp;
    ...
    if(x2temp > L - 1)
    {
        particula[i].position[1] = x2temp - (L-1);
    ...
    {
        particula[i].position[1] = L-1 + x2temp;
        x2temp = particula[i].position[1] + particula[i].velocity[1]*dt + 0.5 * particula[i].old_aceleration[1] * pow(dt,2);
...
double a(double r,double R)
{
    ...
    A = (1.0/pow(sigma,2))*48.*e*(pow((sigma/R),14) -  0.5*pow((sigma/R),8));
